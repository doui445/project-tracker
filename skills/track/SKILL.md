...
- **Tracked**: `docs/project-tracker/STATUS.md` present at the root found.
  ...
  - If its frontmatter has **no** `nested_model` key at all: run the one-time migration check (see `## Sub-projects`, `### Detecting and attaching a sub-project`) — scan for sub-folders with their own `.git`, not already in `~/.claude/project-tracker/trackignore.txt` and not already in `subprojects:`. Once the check has been presented and answered (whatever the outcome — zero candidates found, some adopted, all declined), write `nested_model: "non"`. Never re-run automatically once written.
...
A signal (typically a nested `.git`) triggers a **proposal**, never a silent decision. Reuse the exact 3-way choice from bootstrap step 1 (`## Bootstrapping a new project`), per candidate:
